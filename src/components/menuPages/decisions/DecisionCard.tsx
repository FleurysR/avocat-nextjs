...
export function DecisionCard({ decision, searchTerm }: DecisionCardProps) {
  const objet = decision.objet || "Sans objet";

  return (
    <li>
      <Link
        href={`/Espace-avocat/decisions/${decision.code}`} // <-- Le lien doit inclure 'Espace-avocat'
      >
        <div
        ...
        >
          ...
          <div className="mt-auto pt-4 border-t border-gray-200 dark:border-slate-700 space-y-2">
            ...
            <div className="flex items-center text-sm text-gray-600 dark:text-gray-400">
              ...
              <span className="truncate flex-1">{decision.avocatDefendeur || "-"}</span>
            </div>
          </div>
        </div>
      </Link>
